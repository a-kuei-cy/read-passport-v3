/**
 * 電子閱讀護照系統 v3.7 - Google Apps Script 後端
...
function routeGet_(p) {
  switch (String(p.action || '')) {
    case 'ping':
    case 'health':
      return ok_({ service: 'read-passport-api', version: '3.7', now: nowIso_() });
...
function setupSpreadsheet() {
  ...
  ensureSetting_('systemVersion', '3.6');
...
function getStudentTasks_(b) {
  ...
  const loginStudentId = String(b.studentId || '').trim();
  const loginStudentName = String(b.studentName || '').trim();
  let student = null;

  // v3.7 學生登入改為「班級 + 姓名」。仍保留 studentId 查詢，
  // 讓舊連結或既有程式可繼續使用。
  if (loginStudentId) {
    student = findStudent_(loginStudentId);
    if (!student || !truthyDefaultTrue_(student.active)) throw new Error('找不到啟用中的學生資料');
    if (normalizeClassName_(student.className) !== className) throw new Error('學生資料與班級不符');
  } else {
    if (!loginStudentName) throw new Error('請輸入學生姓名');
    student = findStudentByNameClass_(loginStudentName, className);
  }
...
function getStudentPortal_(b) {
  ...
  const studentId = String(base.student.studentId || '');
  const freeReviews = sheetObjects_(SHEETS.RECORDS)
    .filter(function (r) { return String(r.studentId) === studentId && !String(r.taskId || '').trim(); })
    .sort(function (a, b2) { return dateMs_(b2.createdAt, 0) - dateMs_(a.createdAt, 0); })
    .map(normalizeRecordForJson_);
  return { student: base.student, tasks: base.tasks, freeReviews: freeReviews };
}
...
function submitFreeReview_(b) {
  ...
  const obj = {
    ...
    studentId: String(student.studentId), studentName: String(student.studentName || ''),
